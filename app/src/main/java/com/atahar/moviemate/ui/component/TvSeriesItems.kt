    ...
    Column {
        ...
        LazyVerticalGrid(
            ...
            items(tvSeries.itemCount) { index ->
                tvSeries[index]?.let {
                    TvSeriesItemView(navController, it)
                }
            }
        }
    }

    tvSeries.pagingLoadingState { isLoaded ->
...
@Composable
fun TvSeriesItemView(navController: NavController, item: TvSeriesItem) {
    AsyncImage(
        ...
        contentDescription = "TV Series image"
    )
}
